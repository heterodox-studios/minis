# Euro Pallet (1/10 scale) Technical documentation

The product is a small decorative item. It is 3D printed and distributed in a variety of colours.

## Product Details

**Product name**
: Euro pallet 1/10th scale

**Model number**
: N/A

**SKU**
: SKUs take the form “pallet-pla-CC-XX” where CC is colour code and XX is number of pallets in a pack. For example `pallet-pla-wh-01` is a single white pallet.

**Intended use**
: Home decor

**Target user group**
: Adults

**Technical specifications**
: 3D printed from PLA, contains 4 small captive disc magnets.

**Declaration of Conformity (DoC)**
: [Content available here](./declaration-of-conformity.md) (Signed copy available on request)

**Dimensions**
: 120 mm by 80 mm by 15 mm.

**Weight**
: approx. 50g

## Trade codes

HS code: `3926.90`

EU CN code: `3926 90 97` 

- Plastics and articles thereof; rubber and articles thereof (`Section VII`)
  - Plastics and articles thereof (`Chapter 39`)
    - Other articles of plastics and articles of other materials of headings 3901 to 3914 (`3926`)
      - Other (`3926 90`)


## Bill of Materials

**Body**
: PLA plastic

**Magnets**
: 2mm thick, 5 or 6 mm diameter neodymium magnets. Captive inside the body.


## Safety and Risk Assessment

This product presents no unusual hazards when used for its intended purpose by an adult.

**EU**: There are no specific EU rules applicable to this product. It has been self-assessed to comply with the General Product Safety Regulation. 

## Test Reports and Certificates

None

## User Instructions

### Assembly

No assembly required

### Maintenance and cleaning advice

No maintenance required.

Wipe clean with a soft, damp cloth. Avoid using abrasive cleaners or harsh chemicals.

Do not wash in dishwasher. Do not expose to high heat.

## Safety Information

None required.

## Warnings and precautions

None required.

## Manufacturer

Designed and manufactured by Heterodox AB - [contact details](https://www.heterodox.se/contact)

## Market Surveillance and Incident Reporting Procedures

The product label contains information on the manufacturer. Incidents reported to the manufacturer will be logged and escalated as appropriate.
